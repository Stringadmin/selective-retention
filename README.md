...
> 让 AI 系统"记住"的关键，不在"怎么感知/读取"（信号层），而在"怎么强制执行/写入"（执行层）。

一套关于**重要性驱动的选择性保留**的研究代码与实证：从持续学习的参数保护，到 Agent 记忆的写入层。
...
1. **梯度 hook 冻不住参数。** AdamW 的解耦权重衰减与历史动量会在梯度为零时仍移动参数（实测 drift 9.94e-5）。必须在**优化器层对实际 delta 做投影**才能真正冻结（drift = 0）。
2. **重要性驱动的选择性保留是一个可迁移范式。** 同一个"重要性筛选 → 执行层强制保留/淘汰"的结构，在参数空间（FIP/GPP）和信息空间（IGM）都成立。
3. **RAG 解决了"读"，没解决"写"。** 标准 RAG（强嵌入 + 全量存储）在矛盾解决场景下准确率 **0%**；加上一个写入层（slot 覆盖）后到 **100%**，且记忆量减少到 1/7。

## 决定性实验（headline result）
...
| 标准 RAG + 时间排序（公平补救） | 0.60 | 28 条 |
| **RAG + IGM 写入层（slot 覆盖）** | **1.00** | **4 条** |

![决定性实验](docs/fig2_decisive_experiment.png)

**IGM 不是替代 RAG，而是装在 RAG 前面的写入层**（重要性闸门 + slot 覆盖），复用同一个向量库与嵌入：
...
# 训练可学习的写入打分器（权重完全可解释，held-out ≈ 0.956）
python -m memory_arch.train_scorer

# M0 基线矩阵：4 种记忆方法对比（需要本地 LLM）
...
- 实验在**受控合成数据**上进行，记忆基线用 4B 本地模型——结论方向可信，绝对数字别当真。
- **标准 RAG 就够用时，别加这层。** IGM 的价值只在"全量写入会崩"的场景：大规模、频繁更新、矛盾解决。
